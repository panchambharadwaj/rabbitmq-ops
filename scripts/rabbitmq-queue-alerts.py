...
def check_arg(args=None):
    ...
    parser.add_argument("-threshold", "--arg_threshold_messages_count",
                        type=int,
                        help="RabbitMQ Messages Count Threshold (int). "
                             "Pass 0 to get alerts when there are messages in the queue (used for sideline alerts)",
                        required=True)
    parser.add_argument("-ackRate", "--arg_threshold_ack_rate",
                        type=float,
                        help="RabbitMQ Ack Rate Threshold (float). "
                             "Ignored when 0 is passed to rabbitmq_threshold_messages_count",
                        required=True)
...
def get_rmq_queue_details(host, port, vhost, queue_name, rabbitmq_auth):
    try:
        rmq_response = requests.get(api_rabbitmq_queues % (host, port, vhost, queue_name), auth=rabbitmq_auth)
        if rmq_response.status_code == 200:
            return rmq_response.json()
        else:
            logging.error("Failed to get RabbitMQ queue details. Response: %s. Exiting.." % rmq_response)
            exit()
    except Exception:
        logging.error("Failed to get RabbitMQ queue details. Exception: %s. Exiting.." % traceback.format_exc())
        exit()
...
def main():
    ...
    logging.info("Messages: " + str(rabbitmq_messages_count) + " | Ack rate: " + str(rabbitmq_ack_rate))
    if rabbitmq_threshold_messages_count == 0:
        ...
        if (rabbitmq_messages_count >= rabbitmq_threshold_messages_count) and (
                rabbitmq_ack_rate <= rabbitmq_threshold_ack_rate):
...
if __name__ == '__main__':
    ...
    rabbitmq_host, rabbitmq_port, rabbitmq_vhost, rabbitmq_authentication, rabbitmq_queue_name, rabbitmq_queue_alias, \
        rabbitmq_threshold_messages_count, rabbitmq_threshold_ack_rate, email_from_address, email_from_password, \
        email_to_address, email_host, email_port = check_arg(sys.argv[1:])
